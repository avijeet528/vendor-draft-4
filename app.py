...
h1,h2,h3{font-family:Georgia,'ITC Charter',serif !important;
...
.kpi-box{border-radius:4px;padding:20px 12px;
    text-align:center;color:white;
    border-left:5px solid rgba(255,255,255,0.2);}
.kpi-value{font-size:2.4em;font-weight:700;margin:0;
    line-height:1.1;
    font-family:Georgia,serif !important;}
.kpi-label{font-size:0.72em;font-weight:700;
    opacity:0.9;margin-top:6px;
    letter-spacing:1.2px;text-transform:uppercase;}
.sec-head{font-size:0.73em;font-weight:700;
...
.comp-table thead th{padding:10px 12px;text-align:left;
...
.comp-table tbody td{padding:9px 12px;
    border-bottom:1px solid #EBEBEB;
    vertical-align:middle;word-break:break-word;
    color:#2D2D2D;}
.vbadge{display:inline-block;padding:3px 9px;
...
.verdict-good{background:#FFF5F0;
    border:2px solid #D04A02;border-radius:4px;
    padding:14px 18px;color:#D04A02;font-weight:700;}
.verdict-mid{background:#F5F5F5;
    border:2px solid #4A4A4A;border-radius:4px;
    padding:14px 18px;color:#4A4A4A;font-weight:700;}
.verdict-bad{background:#F0F0F0;
    border:2px solid #7D7D7D;border-radius:4px;
    padding:14px 18px;color:#2D2D2D;font-weight:700;}
.chat-header{background:#2D2D2D;color:white;
    padding:12px 18px;border-radius:6px 6px 0 0;}
.chat-outer{background:#F8F8F8;
    border:1px solid #E0E0E0;border-top:none;
    border-radius:0;padding:14px 14px 6px;
    min-height:320px;max-height:420px;overflow-y:auto;}
.msg-user{background:#D04A02;color:white;
...
.msg-bot{background:white;color:#2D2D2D;
    ...
    padding:9px 14px;margin:5px 0;max-width:84%;
    font-size:0.86em;display:inline-block;
    float:left;clear:both;}
.chat-wrap{overflow:hidden;margin-bottom:3px;}
.filter-bar{background:#2D2D2D;padding:14px 20px;
    border-radius:4px;margin-bottom:18px;
    border-left:4px solid #D04A02;}
.insight-box{background:#FFF5F0;
    border-left:4px solid #D04A02;
    border-radius:0 4px 4px 0;
    padding:11px 16px;margin:10px 0;
    font-size:0.86em;color:#2D2D2D;}
.bucket-header{background:#1A1A1A;color:white;
...
def extract_price_from_bytes(content, ext):
    ...
    if not price or _parse_num(price) <= 0:
        all_n = [h.strip()
                 for h in PRICE_RE.findall(text)
                 if _parse_num(h) >= 1000]
        if all_n:
            price = max(all_n, key=_parse_num)
    return {
        "price":     price,
...
def extract_price_from_file(fp):
    try:
        with open(fp, "rb") as f:
            c = f.read()
        return extract_price_from_bytes(
            c, fp.rsplit(".", 1)[-1])
    except:
...
def price_score(new_price, hist_prices):
    ...
    if mx == mn:
        return 50, "Same as historical average", \
               avg, mn, mx
    score = round(
        (1 - (new_price - mn) / (mx - mn)) * 100, 1)
    score = max(0, min(100, score))
    pct   = round((new_price - avg) / avg * 100, 1)
    lbl   = (
        "{}% BELOW average — COMPETITIVE".format(
            abs(pct))
        if new_price < avg
        else "{}% ABOVE average — REVIEW NEEDED".format(
            abs(pct)))
...
def get_verdict(ps):
    if ps is None:
        return ("⚪ No Data",
                "No comparison data.", C_GREY_DARK)
    if ps >= 70:
        return ("✅ COMPETITIVE",
                "Priced competitively.", C_ORANGE)
    if ps >= 40:
        return ("🟡 AVERAGE",
                "Within range. Negotiate.",
                C_GREY_DARK)
    return ("🔴 HIGH",
            "Above average. Recommend negotiating.",
            C_MID)
...
def _explode(df):
    df2 = df.copy()
    df2["Services List"] = \
        df2["Comments"].apply(_parse_services)
    dfe = df2.explode("Services List").copy()
    dfe.rename(
        columns={"Services List": "Service"},
        inplace=True)
    dfe["Service"] = dfe["Service"].apply(
        lambda x: str(x).strip())
...
def _norm_cols(df):
    ...
    for c in df.columns:
        ...
        if cl == "category" \
                and "Category" not in col_map:
            col_map["Category"] = c
        elif any(k in cl for k in
                 ["vendor", "supplier"]) \
                and "Vendor" not in col_map:
            col_map["Vendor"] = c
        elif ("file name" in cl
              or cl == "filename") \
                and "File Name" not in col_map:
            col_map["File Name"] = c
        elif any(k in cl for k in
                 ["file link", "file url"]) \
                and "File Link" not in col_map:
            col_map["File Link"] = c
        elif any(k in cl for k in
                 ["comment", "service",
                  "description", "scope"]) \
                and "Comments" not in col_map:
            col_map["Comments"] = c
        elif any(k in cl for k in
                 ["price", "cost",
                  "amount", "quoted"]) \
                and "Quoted Price" not in col_map:
...
def load_master_catalog():
    ...
    try:
        raw = pd.read_excel(
            XLS, engine="openpyxl", header=None)
        hr  = 0
...
def ensure_dummy_data():
    if os.path.exists("dummy_catalog.csv"):
        return
    import random
    random.seed(42)
    vendors = [
        "NTT Data", "Dimension Data",
        "Telstra", "Optus", "Vocus", "Datacom"]
    cats = {
        "Cybersecurity": [
            "Endpoint Protection",
            "SIEM Monitoring",
    ...
    for cat, svcs in cats.items():
        for svc in svcs:
            ...
            chosen = random.sample(vendors, n_v)
            for v in chosen:
                base  = random.uniform(
                    20000, 200000)
                price = round(
...
def chatbot_response(
        user_msg, df_master, df_exploded,
    ...
    if any(w in msg for w in
        ...
        return {"type": "text", "text": (
            ...
            "services, prices or categories.".format(
                len(df_master),
    ...
    matched_vendor = next(
        (v for v in all_vendors
         if v.lower() in msg), None)
    matched_services = [
        svc for svc in all_services
        if any(w in msg
               for w in svc.lower().split()
               if len(w) > 3)
    ...
    if matched_vendor and matched_services:
        ...
        if not d.empty:
            ...
            all_p = [
                _parse_num(str(r.get(
                    "Quoted Price",
                    "")).strip())
                for _, r in df_exploded[
                    df_exploded["Service"]
                    == svc].iterrows()
                if _parse_num(str(r.get(
                    "Quoted Price",
                    "")).strip()) > 0]
            v_txt = ""
            if prices and all_p:
                ps2, lbl, avg, mn, mx = \
                    price_score(prices[0], all_p)
                v_txt = \
                    "\n📊 vs market: **{}**".format(
                        lbl)
            return {
    ...
    if matched_vendor:
        d    = df_exploded[
            ...
            == matched_vendor]
        svcs = sorted(
            d["Service"].unique().tolist())
        cats = sorted(
            d["Category"].unique().tolist())
        nq   = len(df_master[
            df_master["Vendor"]
            == matched_vendor])
        prices = []
...
# ════════════════════════════════════════════════════════════
# CATALOG OVERVIEW (reusable)
# ════════════════════════════════════════════════════════════
def render_catalog_overview(
        df_master, df_exploded, label=""):
    ...
    rows3 = [cs_df.iloc[i:i+3]
             for i in range(0, len(cs_df), 3)]
    for chunk in rows3:
        cols = st.columns(len(chunk), gap="medium")
        for ci, (_, rs) in enumerate(
                chunk.iterrows()):
            cn    = rs["Category"]
...
def render_browse_verdict(
    ...
    if sel_cat    != "All":
        d_filt = d_filt[
            d_filt["Category"] == sel_cat]
    if sel_vendor != "All":
        d_filt = d_filt[
            d_filt["Vendor"] == sel_vendor]
    avail_svcs = sorted([
    ...
    chat_history_key = "chat_history_{}".format(
        chat_key)
    ...
    with left_col:
        ...
        if not st.session_state[
            ...
            chat_html += (
                ...
                "of this catalog.<br><br>"
                "<b>Ask me:</b><br>"
                "• Who quoted Cisco Catalyst?<br>"
                "• Compare Palo Alto prices<br>"
        ...
        # Handle file upload
        last_file_key = "last_chat_file_{}".format(
            chat_key)
        if (chat_file is not None
                ...
                != chat_file.name):
            st.session_state[
                last_file_key] = chat_file.name
            fb2_bytes = chat_file.read()
            resp = chatbot_response(
                "uploaded file",
                df_master, df_exploded,
                uploaded_file_bytes=fb2_bytes,
        ...
        if not selected_svcs:
            ...
            if d_sel.empty:
                ...
                if vpm:
                    ...
                    st.markdown(
                        "<div class='insight-box'>"
                        "ℹ️ Price data not available"
                        " for the Master Catalog — "
                        "files are on SharePoint. "
                        "Use <b>Dummy Data</b> tabs "
                        "for full price analysis."
                        "</div>",
                        unsafe_allow_html=True)
...
NO_MASTER = (df_master is None
             or df_master.empty)
NO_DUMMY  = (df_dummy  is None
             or df_dummy.empty)
...
if not NO_MASTER:
    for i, v in enumerate(sorted(
        ...
        vcmap_master[v] = CHART_SEQ[
            i % len(CHART_SEQ)]
if not NO_DUMMY:
    for i, v in enumerate(sorted(
            df_dummy["Vendor"].unique())):
        vcmap_dummy[v] = CHART_SEQ[
            i % len(CHART_SEQ)]
...
with col_d:
    ...
    if not NO_DUMMY:
        ...
        kpi_box(kd1,
                df_dummy["File Name"].nunique(),
                "Quotes",     C_ORANGE)
        kpi_box(kd2,
                df_exp_dummy["Service"].nunique(),
                "Services",   C_DARK)
        kpi_box(kd3,
                df_dummy["Vendor"].nunique(),
                "Vendors",    C_MID)
        kpi_box(kd4,
                df_dummy["Category"].nunique(),
                "Categories", C_GREY_DARK)
    else:
        st.warning(
            "dummy_catalog.csv not found.")
...
(tab_mc_ov, tab_mc_bv,
 tab_dd_ov, tab_dd_bv,
 tab_upload, tab_data,
 tab_upload_cat, tab_vendor) = st.tabs([
    "📁 MC — Catalog Overview",
    "📁 MC — Browse & Verdict",
    "📊 DD — Catalog Overview",
    "📊 DD — Browse & Verdict",
...
with tab_upload:
    ...
    df_exp_bench = (df_exp_dummy
                    if not NO_DUMMY
                    else df_exp_master)
    bench_label  = ("Dummy Data"
                    if not NO_DUMMY
                    else "Master Catalog")
...
with tab_data:
    ...
    src_choice = st.radio(
        ...
        key="dt_src")
    df_dt = (df_master
              if src_choice == "Master Catalog"
              else df_dummy)
    df_dt_use = (df_dt
                  if df_dt is not None
                  else pd.DataFrame())

    if not df_dt_use.empty:
...
with tab_upload_cat:
    ...
    insight(
        ...
        "current one. Supports Excel and CSV.")
    ...
    if cat_file is not None:
        ...
        with st.spinner("Analysing…"):
            try:
                ...
                df_n = _clean_df(df_n)
                df_n["Hyperlink"] = ""
                if "Quoted Price" in df_n.columns:
                    df_n["Quoted Price"] = \
                        pd.to_numeric(
                            df_n["Quoted Price"],
                            errors="coerce"
...
with tab_vendor:
    ...
    if NO_DUMMY:
        ...
        if "Quoted Price" in df_dummy.columns:
            df_dummy["Quoted Price"] = \
                pd.to_numeric(
                    df_dummy["Quoted Price"],
                    errors="coerce").fillna(0.0)
            vt_dd = (
                df_dummy.groupby("Vendor")[
                    "Quoted Price"]
                .agg(["mean", "min",
                      "max", "count"])
                .reset_index())
